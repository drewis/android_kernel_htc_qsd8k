...
int acpuclk_set_rate(unsigned long rate, enum setrate_reason reason)
{
	...
	if (reason == SETRATE_CPUFREQ) {
		...
		if (rc) {
			pr_err("acpuclock: Unable to increase ACPU vdd: %d.\n", (int) rate);
...
#endif
#ifdef CONFIG_MSM_CPU_AVS
	/* if avs init successful. avs will handle voltage changes */
	if (!acpu_avs_init(drv_state.acpu_set_vdd, drv_state.current_speed->acpu_khz))
